...
const AficanProd = () => {
    ...
    const africanProducts = productsByCountry.filter(product => product.country === "Africa");
    const scrollToTop = () => {
        window.scrollTo({ top: 0, behavior: "smooth" });
          ...
          return (
              <>
                  <AboutHeroSection page="African Fruits & Vegetables" />
      
                  {/* Display Products */}
                  <div className="px-6 md:px-20 py-6">
                      <h1 className="text-3xl font-semibold text-center text-gray-800 ">African Products</h1>
      
                      {/* Products Grid */}
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-8 my-10">
                          {africanProducts.map((product, index) => (
                              <div
                                  key={index}
