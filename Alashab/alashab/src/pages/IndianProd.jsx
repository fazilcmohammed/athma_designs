...
import { productsByCountry } from '../assets/constant'; // Import the products
import AboutHeroSection from '../components/AboutHeroSection';
import { Link } from "react-router-dom";


const IndianProd = () => {
  // Filter the products that belong to India
  const indianProducts = productsByCountry.filter(product => product.country === "India");
  const scrollToTop = () => {
  ...
  return (
    <>
      <AboutHeroSection page="Indian Fruits & Vegetables" />

      {/* Display Products */}
      <div className="px-6 md:px-20 py-6">
        <h1 className="text-3xl font-semibold text-center text-gray-800 ">Indian Products</h1>

        {/* Products Grid */}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-8 my-10">
          {indianProducts.map((product, index) => (
            <div
              key={index}
